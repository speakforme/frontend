...
(function(global, factory) {
  ...
  function t$1(el, binding, vnode) {
    ...
    var vm = vnode.context;
    el._vt = el.textContent = (ref$1 = vm.$i18n).t.apply(
      ref$1,
    ...
    while (position < format.length) {
      ...
      if (char === '{') {
        ...
        var type = RE_TOKEN_LIST_VALUE.test(sub)
          ? 'list'
          : RE_TOKEN_NAMED_VALUE.test(sub) ? 'named' : 'unknown';
  ...
  VueI18n.prototype._t = function _t(key, _locale, messages, host) {
    ...
    if (this._isFallbackRoot(ret)) {
      if ('development' !== 'production' && !this._silentTranslationWarn) {
        warn(
          "Fall back to translate the keypath '" + key + "' with root locale."
        );
      }
      /* istanbul ignore if */
      if (!this._root) {
  ...
  VueI18n.prototype._i = function _i(key, locale, messages, host, values) {
    ...
    if (this._isFallbackRoot(ret)) {
      if ('development' !== 'production' && !this._silentTranslationWarn) {
        warn(
          "Fall back to interpolate the keypath '" + key + "' with root locale."
        );
      }
      if (!this._root) {
        throw Error('unexpected error');
  ...
  VueI18n.prototype._tc = function _tc(key, _locale, messages, host, choice) {
    ...
    return fetchChoice(
      (ref = this)._t.apply(ref, [key, _locale, messages, host].concat(values)),
      choice
    );
    var ref;
  };
  ...
  VueI18n.prototype.mergeLocaleMessage = function mergeLocaleMessage(
    ...
    this._vm.messages[locale] = Vue.util.extend(
  ...
  VueI18n.prototype.mergeDateTimeFormat = function mergeDateTimeFormat(
    ...
    this._vm.dateTimeFormats[locale] = Vue.util.extend(
      this._vm.dateTimeFormats[locale] || {},
      format
    );
  };
  ...
  VueI18n.prototype._d = function _d(value, locale, key) {
    ...
    if (
      ...
      warn(
        'Cannot format a Date value due to not support Intl.DateTimeFormat.'
      );
      return '';
    }
  ...
  VueI18n.prototype.mergeNumberFormat = function mergeNumberFormat(
    ...
    this._vm.numberFormats[locale] = Vue.util.extend(
      this._vm.numberFormats[locale] || {},
      format
    );
  };
  ...
  VueI18n.prototype._n = function _n(value, locale, key) {
    ...
    if (
      'development' !== 'production' &&
      !VueI18n.availabilities.numberFormat
    ) {
